...
export const MeetingCard  :React.FC<MeetingCardProps> = ({meetingDetails}) => {
  return (
      <View style={styles.wrapper}>
          {meetingDetails.showMeetingTitle && <Text style={styles.title}>{meetingDetails.title}</Text>}
          <Text style={styles.text} >{ORGANIZED_BY(meetingDetails.organizer) }</Text>
          <Text style={styles.text} >{getTimeInFormat(meetingDetails.start)}-{getTimeInFormat(meetingDetails.end)} </Text>
</View>
  )
}
