 ...
 */
export const MeetingRooms: React.FC<MeetingRoomsProps> = ({  isHorizontal, style }) => {
...
console.log('hitting')
  meetings.length>0&&  meetings?.forEach((meeting: Meetings) => {
    ...
    console.log(meeting.title)
      if (room) {
        if (currentTime < endTime) {
          ...
          const isMeetingOngoing = currentTime >= startTime && currentTime <= endTime;
          const newAvailability = !isMeetingOngoing;
          console.log(meeting.title,meeting.end,endTime)
          console.log(room.name,room.availability,newAvailability)
          if (newAvailability !== room.availability) {
            const updatedRoom = { ...room, availability: newAvailability };
            console.log(room.name,room.availability)
  ...
  return (
      <FlatList
        ...
        contentContainerStyle={isHorizontal? {paddingHorizontal: 20,marginBottom:40}:{paddingVertical:20,marginBottom:40}}
        ItemSeparatorComponent={() => <View style={ isHorizontal?{  width: 15}:{height:15}} />}
        data={rooms}
        renderItem={({item}) => <RoomDetails key={item.id} details={item} style={style} />}
